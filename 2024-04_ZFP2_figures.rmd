...
suppressMessages(library(signs))
...
suppressMessages(library(tukeygrps))
...
```{r}
# Download the data file to run these chunks at:
# https://onlinelibrary.wiley.com/action/downloadSupplement?doi=10.1111%2Ftpj.12790&file=tpj12790-sup-0006-DatasetS1.xlsx

khan_data <- readxl::read_excel("tpj12790-sup-0006-datasets1.xlsx", sheet = 3) |>
  filter(`New AGI ID` == "AT5G57520") |>
  select(2:22) |>
  pivot_longer(
    everything(),
    names_to = "sample",
  ...
  separate_wider_regex(
    sample,
    c(stage = "[:lower:]+", organ = "[:upper:]+")
  ) |>
  mutate(
    rel_abundance = as.numeric(rel_abundance),
    stage = case_match(
      stage,
      "mg" ~ "Mature green stage",
      "g" ~ "Globular stage",
      "h" ~ "Heart stage"
    ),
    organ = case_match(
...
khan_plot <- ggplot(
  ...
  geom_col(fill = pal_zfp[1]) +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.05))
  ) +
  labs(y = "Relative RNA abundance") +
  theme_leo() +
  theme(
    axis.title.y = element_blank(),
    axis.title.x = element_markdown(),
  ...
  ggforce::facet_col(vars(stage), scales = "free_y", space = "free") +
  coord_flip()
...
luc_myb_plot <- function(prom) {
  ...
  ggplot(
    ...
    scale_x_discrete(labels = c("Reporter<br>only", "ZFP2")) +
    theme_leo() +
    theme(
...
```{r}
tree <- read.tree("ZFO_newick.nwk")
tree$tip.label <- str_remove_all(tree$tip.label, "'")
tree$node.label <- round(as.numeric(tree$node.label) * 100, digits = 0)

tree_plot <- ggtree(tree, layout = "circular", linewidth = 0.4) +
  geom_tiplab(
    size = ggtext_size * 0.75,
    aes(
      color = ifelse(label == "AT5G57520 (ZFP2)", pal_zfp[3], "black"),
      angle = angle
      )
  ) +
  geom_nodelab(
...
plot_spacer() +
  ...
  seed_oil_plot +
  gluc_plot(
    "Long chain aliphatic glucosinolates (C6–C8)", 
    "10 seeds from one silique"
    ) + labs(y = "Long chain glucosinolates (nmol seed<sup>−1</sup>)") +
...
BCCCC
"
seed_count_plot + fa_timeline_plot + gluc_supp +
  plot_layout(design = design) +
  plot_annotation(tag_levels = "A") &
  theme(plot.tag = element_text(size = 9, face = "bold", family = "Myriad"))
...
luc_myb_plot("proSND2") + 
  luc_myb_plot("proMYB63") + 
  plot_layout(axes = "collect", axis_titles = "collect_y") +
  luc_vp_plot + 
  luc_mut_plot +
  free(tree_plot) +
  plot_layout(design = design) 
